    ...
    <div className="space-y-8 max-w-6xl mx-auto px-4 sm:px-6 py-8 fade-in-up">
      ...
      <div className="bg-bg-card border border-border-primary/80 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-bg-surface border border-border-primary/60 rounded-xl p-3.5 space-y-1">
            <div className="text-[10px] font-mono text-text-muted uppercase">Sẵn sàng thi</div>
            <div className="text-2xl font-mono font-bold text-brand-info">{plan.readinessScore}/100</div>
            <div className="text-[10px] text-text-muted">Chỉ số tự tin làm bài</div>
          </div>

          <div className="bg-bg-surface border border-border-primary/60 rounded-xl p-3.5 space-y-1">
            <div className="text-[10px] font-mono text-text-muted uppercase">Độ thông thạo</div>
            <div className="text-2xl font-mono font-bold text-brand-success">{plan.masteryScore}%</div>
            <div className="text-[10px] text-text-muted">Tỷ lệ chính xác tổng thể</div>
          </div>

          <div className="bg-bg-surface border border-border-primary/60 rounded-xl p-3.5 space-y-1">
            <div className="text-[10px] font-mono text-text-muted uppercase">Nhiệm vụ hôm nay</div>
            <div className="text-xs font-semibold text-text-primary truncate">{plan.todayGoal}</div>
            <div className="text-[10px] font-mono text-brand-info">{plan.estimatedStudyTime} phút dự kiến</div>
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono text-text-muted">7 Chương trọng tâm</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {plan.chapterStatuses.map((ch) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-bold text-brand-info bg-brand-info/10 px-2 py-0.5 rounded">
                  {ch.code}
                </span>

                {ch.status === "COMPLETED" && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-brand-success">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Hoàn thành</span>
                  </span>
                )}
                {ch.status === "WEAK" && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-brand-warning">
                ...
                {ch.status === "READY" && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-mono text-brand-info">
      ...
      <div className="bg-bg-card border border-border-primary/80 rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-border-primary/60 pb-3">
          ...
          <span className="text-[10px] font-mono text-text-muted">Mục tiêu tuần: {plan.weeklyGoal}</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-2.5">
          {plan.weeklyPlan.map((item, idx) => (
            <div key={idx} className="p-3 bg-bg-surface border border-border-primary/60 rounded-xl space-y-2 text-xs flex flex-col justify-between">
              <div className="space-y-1">
                <div className="flex items-center justify-between text-[10px] font-mono font-bold text-brand-info">
                  <span>{item.dayTitle}</span>
                  <span>{item.estimatedMinutes} phút</span>
                </div>
                <div className="font-medium text-text-primary leading-tight text-[11px] line-clamp-2">{item.focus}</div>
              </div>

              <div className="pt-2 border-t border-border-primary/40 space-y-1 text-[10px] text-text-muted">
                <div className="flex justify-between">
                  <span>Mục tiêu:</span>
                  <span className="font-mono text-brand-success">{item.targetMastery}%</span>
                </div>
                <div className="text-[10px] italic text-text-muted line-clamp-2">{item.suggestedActivity}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="space-y-2.5">
              {plan.studyDebt.map((debt, idx) => (
                <div key={idx} className="p-3 bg-bg-surface border border-border-primary/60 rounded-xl space-y-1 text-xs">
                  ...
                  <p className="text-[11px] text-text-muted leading-relaxed">{debt.reason}</p>
                  <div className="pt-1 text-[11px] font-medium text-brand-info">{debt.suggestedAction}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-bg-card border border-border-primary/80 rounded-2xl p-5 space-y-3">
          ...
          <div className="space-y-2 text-xs text-text-muted">
            <div className="p-2.5 bg-bg-surface border border-border-primary/60 rounded-xl space-y-1">
              ...
              <p className="text-[11px] leading-relaxed">{plan.explainability.reason}</p>
            </div>

            <div className="p-2.5 bg-bg-surface border border-border-primary/60 rounded-xl space-y-1 text-[11px] font-mono">
