...
import { Activity, ShieldCheck, TrendingUp, Layers, RefreshCw, CheckCircle2, XCircle, FileText, Search, Compass, Brain, Info } from "lucide-react";
import { 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 font-sans">
      ...
      <div className="bg-gradient-to-br from-bg-card via-bg-surface to-bg-card border border-border-primary rounded-2xl p-6 sm:p-8 shadow-sm relative overflow-hidden">
        <div className="absolute -right-12 -bottom-12 w-64 h-64 bg-brand-info-bg rounded-full blur-3xl pointer-events-none" />
...
{/* Trước 28/07/2026 tiêu đề này là "Observability & Self-Improving Platform",
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-bg-surface/80 p-4 rounded-xl border border-border-primary/80 backdrop-blur-sm">
            ...
            >
              <RefreshCw className={`w-4 h-4 ${isExecutingJob ? "animate-spin" : ""}`} />
        ...
        <button
          ...
          className={`px-4 py-2 text-xs font-semibold rounded-t-lg transition flex items-center gap-2 whitespace-nowrap border-b-2 ${
            activeTab === "observatory"
              ? "border-brand-info text-brand-info bg-brand-info-bg"
      ...
      {activeTab === "observatory" && (
        <div className="space-y-6 fade-in">
          ...
          <div className="bg-bg-card border border-border-primary rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="p-3 bg-bg-surface rounded-lg border border-border-primary tabular-nums text-xs text-text-primary overflow-x-auto">
            ...
            <div className="bg-bg-card border border-brand-success/40 rounded-xl p-4 shadow-sm flex items-start gap-3 fade-in">
              ...
              <div className="space-y-1 text-xs">
                <div className="font-bold text-text-primary flex items-center gap-2">
                  ...
                  <span className="tabular-nums text-text-muted">({lastJobResult.executedAt})</span>
                </div>
                  ...
                  {deadConcepts.slice(0, 4).map((dc, idx) => (
                    <div key={idx} className="p-3 bg-bg-surface rounded-lg border border-border-primary/80 space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-2xs tabular-nums font-semibold text-brand-warning bg-brand-warning-bg px-2 py-0.5 rounded">
                          {dc.questionCount} Câu hỏi (Rủi ro: {dc.exposureRisk})
                        </span>
                      </div>
                      <p className="text-2xs text-text-muted">{dc.reason}</p>
                      <div className="text-2xs tabular-nums text-brand-info bg-brand-info-bg p-2 rounded border border-brand-info/10">
...
<span>Khái niệm chưa khai thác</span>
              ...
              <span className="text-xs tabular-nums font-semibold px-3 py-1 bg-brand-warning-bg text-brand-warning rounded-full border border-brand-warning/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {deadConcepts.map((dc, idx) => (
                <div key={idx} className="p-4 bg-bg-surface rounded-xl border border-border-primary space-y-3">
                  ...
                  <div className="text-2xs tabular-nums text-text-muted bg-bg-surface/80 p-2 rounded border border-border-primary/60">
...
<span>Khái niệm bị lạm dụng</span>
              ...
              <span className="text-xs tabular-nums font-semibold px-3 py-1 bg-brand-info-bg text-brand-info rounded-full border border-brand-info/20">
              ...
              <div className="space-y-3">
                {overusedConcepts.map((oc, idx) => (
                  <div key={idx} className="p-4 bg-bg-surface rounded-xl border border-border-primary flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="text-2xs tabular-nums text-brand-info bg-bg-card p-2.5 rounded-lg border border-border-primary max-w-md">
              ...
              {distractorReports.slice(0, 3).map((report, idx) => (
                <div key={idx} className="p-4 bg-bg-surface rounded-xl border border-border-primary space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs tabular-nums font-semibold text-brand-success bg-brand-success-bg px-2.5 py-0.5 rounded">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
                    {report.optionsHealth.map((opt, oIdx) => (
                      <div key={oIdx} className={`p-2.5 rounded-lg border text-xs space-y-1 ${
                        opt.isCorrect 
                          ? "bg-brand-success-bg border-brand-success/30" 
                          : opt.status === "DEAD_DISTRACTOR"
                          ? "bg-brand-warning-bg border-brand-warning/30"
              ...
              {blueprintPerf.map((bp, idx) => (
                <div key={idx} className="p-4 bg-bg-surface rounded-xl border border-border-primary space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-2xs tabular-nums font-bold px-2 py-0.5 rounded ${
                      bp.status === "BALANCED" ? "bg-brand-success-bg text-brand-success" : "bg-brand-warning-bg text-brand-warning"
                    }`}>
                ...
                <div key={idx} className="p-3.5 bg-bg-surface rounded-xl border border-border-primary flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                  ...
                  <div className="flex items-center gap-4 text-right">
                    <div>
                      ...
                      <div className="tabular-nums font-bold text-text-primary">{dd.designedDifficultyLabel}</div>
                    </div>

                    <div>
                      <div className="text-text-muted text-2xs">Tỷ lệ Lỗi Thực tế</div>
                      <div className="tabular-nums font-bold text-brand-warning">{dd.actualErrorRatePct}% Sai</div>
                    </div>

                    <div className={`px-2.5 py-1 rounded tabular-nums font-bold text-2xs ${
                      dd.driftType === "CALIBRATED" ? "bg-brand-success-bg text-brand-success" : "bg-brand-warning-bg text-brand-warning"
                    }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recommendations.map((rec, idx) => (
                <div key={idx} className="p-4 bg-bg-surface rounded-xl border border-border-primary space-y-3">
                  ...
                  <div className="text-2xs tabular-nums text-brand-info bg-bg-card p-2 rounded border border-border-primary">
                    {rec.explainability}
                  </div>
                </div>
              ))}
...
<span>Nợ kỹ thuật học thuật</span>
              ...
              <span className="text-xs tabular-nums font-bold px-3 py-1 bg-brand-warning-bg text-brand-warning rounded-full">
                ...
                <div key={idx} className="p-3.5 bg-bg-surface rounded-xl border border-border-primary flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xs tabular-nums px-2 py-0.5 rounded bg-brand-warning-bg text-brand-warning">
                  ...
                  <div className="text-2xs tabular-nums text-text-muted bg-bg-card p-2 rounded border border-border-primary max-w-xs">
...
<span>Đánh giá mức độ sẵn sàng phát hành</span>
              ...
              <span className={`text-xs tabular-nums font-bold px-3 py-1 rounded-full ${
                readiness.isReady ? "bg-brand-success-bg text-brand-success" : "bg-brand-warning-bg text-brand-warning"
              }`}>
              ...
              {changelog.map((item, idx) => (
                <div key={idx} className="p-3 bg-bg-surface rounded-xl border border-border-primary flex items-start justify-between gap-4 text-xs">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="tabular-nums font-bold text-brand-info">[{item.type}]</span>
                      <span className="font-semibold text-text-primary">{item.author}</span>
                      <span className="text-text-muted tabular-nums text-2xs">({item.timestamp})</span>
                    </div>
                    <p className="text-text-muted">{item.details}</p>
                  </div>

                  <div className="tabular-nums font-bold text-brand-success">
